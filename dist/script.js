...
const forms = state => {
  ...
  (0,_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('input[name="user-phone"]');
  const message = {
    loading: 'Загрузка...',
...
const images = () => {
  ...
  workSection.addEventListener('click', e => {
    ...
    if (target && target.matches('div.popup')) {
      //выражение в скобках равно клику на подложку
      imgPopup.style.display = 'none';
    }
  });
};
...
const timer = (selector, deadline) => {
  ...
  function setClock(selector, endtime) {
    ...
    function updateClock() {
      ...
      days.textContent = getZero(t.days), hours.textContent = getZero(t.hours), minutes.textContent = getZero(t.minutes), seconds.textContent = getZero(t.seconds);
      if (t.total <= 0) {
